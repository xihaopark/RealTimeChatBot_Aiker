...
class VTXAIPhoneSystem:
    ...
    def _handle_invite(self, message, addr, call_id):
        ...
        # 启动RTP和AI对话
        self._start_rtp_and_ai(message, addr, rtp_port, call_id)
    ...
    def _start_rtp_and_ai(self, invite_message, addr, rtp_port, call_id):
        """启动RTP和AI对话"""
        print(f"🎵 启动RTP和AI对话: {addr[0]}:{rtp_port}")
        
        # 解析INVITE消息中的SDP获取远程RTP端口
        remote_rtp_port = self._parse_remote_rtp_port(invite_message)
        if not remote_rtp_port:
            print("❌ 无法解析远程RTP端口，使用默认值")
            remote_rtp_port = 10000
        
        print(f"🎵 远程RTP端口: {remote_rtp_port}")
    ...
    def _parse_remote_rtp_port(self, invite_message):
        """从INVITE消息中解析远程RTP端口"""
        try:
            # 查找SDP部分
            sdp_start = invite_message.find('\r\n\r\n')
            if sdp_start == -1:
                return None
            
            sdp_text = invite_message[sdp_start+4:]
            print(f"📝 解析SDP: {sdp_text[:200]}...")
            ...
            if sdp['media']:
                ...
                remote_port = audio_media.get('port')
                if remote_port:
                    print(f"✅ 找到远程RTP端口: {remote_port}")
                    return remote_port
            
            return None
            
        except Exception as e:
            print(f"❌ 解析远程RTP端口失败: {e}")
            return None
        ...
        try:
            ...
            def audio_callback(audio_data):
                if rtp_handler and rtp_handler.running:
                    # 检查音频数据长度，判断是否为预生成的完整音频文件
                    if len(audio_data) > 1000:  # 大于1000字节认为是完整音频文件
                        print(f"📤 发送预生成音频: {len(audio_data)} 字节")
                        # 预生成的音频已经是μ-law格式，直接按160字节分包发送
                        packet_size = 160
                        ...
                        # 小数据包，直接发送（来自实时TTS的分包数据）
                        if len(audio_data) == 160:
                            rtp_handler.send_audio(audio_data, payload_type=0)
                        else:
                            # 需要分包的实时TTS数据
                            packet_size = 160
                            for i in range(0, len(audio_data), packet_size):
