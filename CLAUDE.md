...
### 技术栈升级
- **STT**: Vosk (替换RealtimeSTT) - 轻量、精准、无音频依赖
- **LLM**: Llama.cpp Server (替换Transformers) - 高并发、量化优化
- **TTS**: Piper (替换RealtimeTTS) - 极速合成、CPU优化
...
│   ├── llm_service.py       # Llama.cpp LLM服务
│   ├── stt_service.py       # Vosk STT服务
│   └── requirements.v2.txt  # 精简依赖列表
├── services/                # AI服务目录
│   ├── piper/              # TTS服务
│   ├── llama.cpp/          # LLM服务
│   └── vosk/               # STT服务
├── setup_aiker_v2.sh       # 一键部署脚本
└── test_v2_performance.py  # 性能测试脚本
```
